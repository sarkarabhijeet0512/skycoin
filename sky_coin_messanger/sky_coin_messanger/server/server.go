...
func (s *Server) Msg(c *clients.Client, args []string) {
	...
	// encrypting using private and public key
	msg := strings.Join(args[1:], " ")
	bit := 2048
	priv, pub := cipher.GenerateKeyPair(bit)
	...
	cipherTExt := cipher.EncryptWithPublicKey([]byte(msg), pubKey)
	plainText := cipher.DecryptWithPrivateKey(cipherTExt, PrivKey)
	c.Room.Broadcast(c, c.Name+": "+string(plainText))
}
